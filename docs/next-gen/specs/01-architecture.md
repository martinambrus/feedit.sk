...
| Language detection | **franc** (full build), restricted whitelist ([spec 03 §7](./03-ingestion.md)) | |
...
│       ├── src/
│       │   ├── cli.ts            # commander entry: ingest-sample, serve-rating, run, report, tune
│       │   ├── rating-server/    # tiny Fastify app + static HTML for raters and facet labelling
│       │   ├── experiments/      # one file per experiment (E1…E6)
...
├── packages/
│   ├── shared/                   # config (zod env), DTO schemas, error types, ids, time utils, logger factory
│   ├── db/                       # Drizzle schema, migrations, repositories, RLS helpers (spec 02)
│   ├── feeds/                    # fetch (SSRF-safe), parse, canonicalize, dedup keys, extract, lang-detect, OPML (spec 03)
│   ├── engine/                   # DecisionEngine + TypeSafe/Degraded/LlmFallback/Laya engines, breaker, budget (spec 04)
...
│   └── testing/                  # fixtures (feeds, HTML pages, Jev responses), factories, test DB helpers
├── infra/
...
- `packages/db` imports only `shared`.
- `packages/feeds`, `packages/translate`, `packages/questions` import only `shared` (and `db` types where stated).
- `packages/engine` imports `shared` and `questions` (types only).
- `packages/ranker` imports `shared` and `questions` (types only). It is **pure**: no I/O, no DB access, all inputs passed in.
...
`packages/shared/src/config.ts` with zod. An invalid config makes the process exit with a readable error.
...
| `DATABASE_URL_WORKER` | — (required for worker) | worker | connection as role `feedit_worker` (BYPASSRLS) |
| `DATABASE_URL_MIGRATE` | — (required for migrate) | migrate script | connection as role `feedit_owner` |
...
| `MAIL_TRANSPORT` | `smtp` (`log` in development/test) | api | `log` writes emails to the console and keeps the last one for the test-only endpoint |
| `SMTP_URL` | — (required in production) | api | `smtp://user:pass@host:587` |
| `MAIL_FROM` | `FeedIt <no-reply@localhost>` | api | |
| `SIGNUP_MODE` | `invite` | api | `invite` / `open` / `closed` |
...
| `LIBRETRANSLATE_URL` | `http://libretranslate:5000` | worker | tier-1 translation |
...
| `ADMIN_EMAILS` | — | api | comma list; these users get `role=admin` on first login |
...
A **migration job** (`pnpm --filter @feedit/db migrate`) runs before `api` and `worker` start
...
- **Errors:** throw subclasses of `AppError` (`packages/shared/src/errors.ts`) with a stable `code`
  (e.g. `FEED_UNREACHABLE`, `QUOTA_EXCEEDED`, `ENGINE_UNAVAILABLE`). The API maps them to HTTP status
...
- **SQL:** prefer Drizzle query builders. Use raw SQL (`sql```) for window functions, `pg_trgm`, and
  ...
  that takes a `TenantContext` ([spec 02 §5](./02-data-model.md)).
...
| Level | Command | What | Required for |
|---|---|---|---|
...
| Unit | `pnpm test` | pure functions, handlers with fakes | every task |
| Integration | `pnpm test:int` | real Postgres (compose.test.yml), migrations applied, RLS on | tasks touching `db`, handlers, API routes |
| E2E | `pnpm --filter web e2e` | Playwright smoke: login → add feed → rate → lanes | M6 and later |
| Eval | `pnpm --filter eval run …` | live Jev calls against the golden set | M3, and any change to questions or thresholds |
...
4. Start Postgres from `compose.test.yml` → `pnpm test:int`
5. `pnpm build`
6. (from M6 on) Playwright smoke against `compose.dev.yml`
...
- `pnpm i && docker compose -f infra/compose.dev.yml up -d postgres libretranslate`
- `pnpm db:migrate && pnpm db:seed` (seeds the taxonomy, the card library and an admin user)
...
- Work one task at a time, in the order and parallel lanes given in PLAN.md. A task is done only when
  its "Done when" list is fully satisfied and `pnpm typecheck && pnpm lint && pnpm test` pass
  (plus `pnpm test:int` if the task touches db, handlers or routes).
- Commit per task: `<task-id>: <summary>` (e.g. `M1-T4: SSRF-safe fetch dispatcher`).
