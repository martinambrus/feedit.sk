...
- **Secrets:** `/opt/feedit/.env` (mode 600): the superuser password and the passwords for the three roles, `TYPESAFE_API_KEY`,
  `OLLAMA_API_KEY`, `SMTP_URL`, `SESSION_PEPPER`, `METRICS_TOKEN`, `ADMIN_EMAILS`, and backup encryption/bucket settings. Compose passes each
  service only its declared variables; the API never receives owner/worker credentials, model keys
  or backup credentials. Use stdin/secret files where possible; do not echo interpolated Compose
  config or put secrets in command arguments. Document credential rotation and session invalidation.
...
(RTO) ≤ 4 h, verified on the target-size dataset. These are launch assumptions requiring owner
acceptance; tighter RPO requires WAL archiving/PITR and is not provided by a daily dump.
...
- Upload encrypted artifacts to an S3-compatible bucket and verify object size/checksum before
  ...
  from those keys. Back up encrypted `.env` on change plus deploy/restore instructions; Caddy state
...
- Verify expected schemas/extensions/migration version, FK validity, sequences, active-set seeds,
  ...
  login/read/rank/outbox smoke tests. Record measured restore duration and restored backup age.
...
4. Do not immediately resume restored `pgboss` jobs: cancel/expire stale active leases through the
   ...
   Rebuild eligible work, skipping deleted users and superseded revisions. Disable delivery of old
...
**Retention precedence:** bookmarks, current non-null ratings, explicit labels and golden-set
references protect an article from the 90-day purge; historic undone feedback does not. Retention
windows are maxima, not promises that child rows survive deletion of their parent. Extraction after
30 days does not run solely to repopulate deliberately purged full bodies. `body_lead` and translated
...
private-card answers/examples, sessions/codes, user-specific outbox/jobs and cost attribution. Remove
...
| `house.reconcile` | `*/10 * * * *` | bounded repair of stranded pipeline/outbox/match work and orphaned leases; enqueue rank for due `user_article.next_rank_at`; nightly UTC window also refreshes feed subscribers/cards, cluster counts and `lang_hint` with persistent progress cursors |
...
| `house.purge-engine-calls` | `0 4 * * *` | delete `engine_calls` older than 180 days and `feedback_events` older than 365 days; clean delivered outbox rows and invoke bounded queue-history maintenance per §5 |
...
and active-user/retention checks.
...
- **Third-party images:** default `loadRemoteImages = false`; do not create remote `src`/CSS URLs
  (including feed icons) until the user explicitly opts in. Explain that loading discloses IP/request
