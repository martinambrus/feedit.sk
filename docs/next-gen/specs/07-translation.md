...
Before each tier-2 HTTP attempt the handler calls `router.reserveExternalCall(...)` to atomically
reserve the estimated spend and a tier-2
daily-cap slot through the shared budget store (spec 04 §6). `canSpend` alone is advisory, not a
...
1. Load `title`, `excerpt` (first 600 chars) and `body_lead` (≤ 1,500) plus `lang` and
   `content_revision`. Translation rows are valid only for that revision. Replayed jobs reuse a
   completed current-revision row; an article edit makes earlier rows ineligible. A late result may
   only be committed if the article still has the revision read at dispatch (spec 02).
...
- Publish translated fields atomically as one validated pair; on partial/weak/failed translation
  ...
  affected users' rank revisions and invalidate incompatible personal models. Switching back to
  `as_written` follows the same digest/invalidation policy; never mix answers made with different
...
- A card mode/text-digest change rematches affected pairs and invalidates model compatibility;
