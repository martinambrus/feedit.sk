...
foreach ($mongo->{MONGO_DB_NAME}->accounts->find([ 'active' => 1 ], [
  ...
  foreach ($mongo->{MONGO_DB_NAME}->{'feeds-' . $user->short_id}->find([ 'tiers_training_check' => [ '$exists' => false ] ], [ 'projection' => ['_id' => 1] ]) as $feed_data) {
    $feeds_count++;
    try {
      // get number of trained links and all links
      $all_docs = $mongo->{MONGO_DB_NAME}->{'training-' . $user->short_id}->countDocuments( [ 'feed' => $feed_data->_id ] );

      // check that we have at least 200 trained links
      if ($all_docs >= 200) {
        $trained_docs    = $mongo->{MONGO_DB_NAME}->{'training-' . $user->short_id}->countDocuments( [
          'feed'    => $feed_data->_id,
          'trained' => 1
        ] );
        ...
        if (($trained_docs / $all_docs) * 100 >= 32) {
          $trained_up_docs = $mongo->{MONGO_DB_NAME}->{'training-' . $user->short_id}->countDocuments( [ 'feed' => $feed_data->_id, 'trained' => 1, 'rated' => 1 ] );
          ...
          if (($trained_up_docs / $trained_docs) * 100 >= 4) {
            ...
            $feed_average_score = $mongo->{MONGO_DB_NAME}->{'training-' . $user->short_id}->aggregate([
              [
                '$match' => [
                  'feed' => $feed_data->_id,
                  'trained' => 0,
            ...
            if ($feed_average_score > 163) {
              $mongo->{MONGO_DB_NAME}->{'feeds-' . $user->short_id}->updateOne( [ '_id' => $feed_data->_id ], [
                '$set' => [
